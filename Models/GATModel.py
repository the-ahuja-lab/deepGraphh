...
class GatModel:
  def __init__(self,job_id,created,email,job_name,status,mode,residual,n_tasks,activation,n_classes,dropout,pred_dropout,no_atom_features,graph_attention_layers,hidden_feature,alpha,self_loop,n_attention_heads,agg_modes,learning_rate,epoch,csv_name):
    self.job_id = job_id
    ...
    self.n_attention_head = n_attention_heads
  ...
  def load_csv(self):
    ...
    database.updateCurrentStatus(self.job_id,"gat",2)
    ...
    database.updateCurrentStatus(self.job_id, "gat", 3)
  ...
  def fit_predict(self):
    ...
    model = GATModel(mode='classification', n_tasks=1, batch_size=32, learning_rate=0.001,n_classes = self.no_classes)
    model.fit(train_dataset, nb_epoch=50)

    database.updateCurrentStatus(self.job_id, "gat", 4)
    predict_label = model.predict(test_dataset)
    self.predict_label = predict_label

    database.updateCurrentStatus(self.job_id, "gat", 5)
    predict_train = model.predict(train_dataset)
    self.predict_train = predict_train

    database.updateCurrentStatus(self.job_id, "gat", 6)


    label = [0] * len(predict_label)
  ...
  def results(self):
    ...
    database.updateCurrentStatus(self.job_id, "gat", 7)
    ...
    for i in range(self.no_classes):
      fpr[i], tpr[i], _ = roc_curve(makingtestLabel[:, i], predict_label[:, i])
    ...
    print(fpr)
    print(tpr)
    database.updateCurrentStatus(self.job_id, "gat", 8)
    ...
    database.updateCurrentStatus(self.job_id, "gat", 9)
    return self.status