...
class AttentiveFPmodel:
  ...
  def fit_predict(self):
    ...
    model = AttentiveFPModel(mode='classification', n_tasks=1, batch_size=32, learning_rate=0.001,n_classes = self.no_classes)
    model.fit(train_dataset, nb_epoch=50)

    database.updateCurrentStatus(self.job_id, "attentive", 4)
    predict_label = model.predict(test_dataset)
    self.predict_label = predict_label
    
    database.updateCurrentStatus(self.job_id, "attentive", 5)
    predict_train = model.predict(train_dataset)
    database.updateCurrentStatus(self.job_id, "attentive", 6)
  ...
  def results(self):
    ...
    database.updateCurrentStatus(self.job_id, "attentive", 7)
    ...
    print(fpr)
    print(tpr)
    database.updateCurrentStatus(self.job_id, "attentive", 8)
    ...
    database.updateCurrentStatus(self.job_id, "attentive", 9)
    return self.status
